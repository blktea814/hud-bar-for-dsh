...
- **悬浮 HUD 条** — 网页内始终置顶、可拖拽、可缩放
- **实时转录** — Markdown 渲染、流式输出光标、工具调用紧凑展示
- **切换器** — 工作区 / 会话 / 模型（按提供方分组）/ 推理强度，输入框上方亦有内联切换
- **发送与停止生成** — agent 运行中主按钮变为"停止生成"，与网页版一致
- **待审批提示** — 任一会话有待审批的权限请求时显示黄色提示条，提醒回主界面批准
- **画中画外置** — Chrome/Edge 116+ 可弹出为 OS 级置顶窗口，盖在其他应用之上
...
- 画中画外置需要 Chrome/Edge 116+（`documentPictureInPicture`），其他浏览器回退为页内悬浮条
- 模型切换下一轮生效，并保存为默认选择
